...
from master_utils import crps, real_to_cdf, preprocess, rotation_augmentation, shift_augmentation
...
def train():
    ...
    for i in range(nb_iter):
        ...
        model_systole.save_weights('master/weights_systole.hdf5', overwrite=True)
        model_diastole.save_weights('master/weights_diastole.hdf5', overwrite=True)

        # for best (lowest) val losses, save weights
        if val_loss_systole < min_val_loss_systole:
            min_val_loss_systole = val_loss_systole
            model_systole.save_weights('master/weights_systole_best.hdf5', overwrite=True)

        if val_loss_diastole < min_val_loss_diastole:
            min_val_loss_diastole = val_loss_diastole
            model_diastole.save_weights('master/weights_diastole_best.hdf5', overwrite=True)

        # save best (lowest) val losses in file (to be later used for generating submission)
        with open('master/val_loss.txt', mode='w+') as f:
