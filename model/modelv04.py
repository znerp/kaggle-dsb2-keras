### 22.01.2019
# Model v04 -> segmentation network, 
# U-net with variable input size
# one layer deeper than modelv03
# added Conv2D after Upsampling in block_up
...
def block_up(input_tensor, fwd_tensor, filters, kernel_size=(3,3), padding='same', f_act='relu'):
    ...
    try:
        ...
        assert(X.shape == fwd_tensor.shape) # check that tensors even have the exact same size
...
def segnet_single_slice(input_shape=(1,128,128), base_n_filters= 16, keep_prob=1):
    """
    ...
    base_n_filters: how many filters does the first layer of the u-net have (layers of
                    deeper blocks have double as many filters)

    """
    p_drop = 1-keep_prob

    nFilters = [base_n_filters*2**n for n in range(5)] # one fourth of the filters of the actual U-net
    ...
    X_fwd_4 = Dropout(p_drop)(X)
    # print(X.shape)

    X = block_down(X_fwd_4, filters=nFilters[4])
    X = Dropout(p_drop)(X)

    ### Upsampling
    # (Upsamling--> CONV -> CONV-> DROPOUT)x3
    X = block_up(X, X_fwd_4, filters=nFilters[3])
    X = Dropout(p_drop)(X)
    # print(X.shape)
    ...
    model = Model(inputs = X_input, outputs = X, name='ss_segnet_v02')
